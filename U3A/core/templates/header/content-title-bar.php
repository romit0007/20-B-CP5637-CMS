<?php
  $title_align_class = $single_page_title_align = $values = $titlebar_bg = $shop_titlebar_bg = $shop_title_bar_text_align = $shop_page_particles = $blog_single_sidebar = '';
     ...
     <div class="row blog-page-heading <?php echo esc_html( $title_align_class ); ?>">
        ...
        <div class="container">
          ...
          <?php elseif ( is_search() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_search_query_title', esc_html__("Search results for", "leadengine") ); ?>: <?php the_search_query();  ?></h1>
          <?php elseif ( is_category() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_single_cat_title', esc_html__("Currently browsing", "leadengine") ); ?>: <?php single_cat_title(); ?></h1>
            <?php the_archive_description( '<div class="taxonomy-description">', '</div>' ); ?>
          <?php elseif ( is_tag() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_single_tag_title', esc_html__("All posts tagged", "leadengine") ); ?>: <?php single_tag_title(); ?></h1>
          <?php elseif ( is_author() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_author_archive_title', esc_html__("All posts by", "leadengine") ); ?> <?php echo esc_attr( get_userdata( get_query_var( 'author' ) )->display_name); ?></h1>
          <?php elseif ( is_day() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_day_archive_title', esc_html__("Posts archive for", "leadengine") ); ?> <?php echo get_the_date('F jS, Y'); ?></h1>
          <?php elseif ( is_month() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_month_archive_title', esc_html__("Posts archive for", "leadengine") ); ?> <?php echo get_the_date('F, Y'); ?></h1>
          <?php elseif ( is_year() ) : ?>
            <h1 class="section-heading"><?php echo apply_filters( 'kd_year_archive_title', esc_html__("Posts archive for", "leadengine") ); ?> <?php echo get_the_date('Y'); ?></h1>
...
<?php elseif( !is_404() && !is_singular( 'portfolio' ) ) : ?>
  <?php if ( empty($page_showhide_title_section) && !is_single() ) : ?>
    <header class="entry-header single-page-header <?php if ( leadengine_get_option( 'tek-topbar' ) == '1') { echo esc_html('with-topbar'); } ?>" <?php if ($titlebar_bg != '') { echo 'style="background-color: '.$titlebar_bg.';"'; } ?>>
