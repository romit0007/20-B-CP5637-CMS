...
// ------------------------------------------------------------------------
	...
	define( 'LEADENGINE_THEME_VERSION', '2.0' );
...
// ------------------------------------------------------------------------
	...
	require_once LEADENGINE_THEME_DIR . 'core/theme-pagemeta.php';
...
// ------------------------------------------------------------------------
	...
	if( ! function_exists( 'leadengine_enqueue_scripts' ) ) {
		function leadengine_enqueue_scripts() {
			...
			if ( leadengine_get_option( 'tek-typekit' ) ) {
	...
	if( ! function_exists( 'leadengine_enqueue_admin' ) ) {
	    ...
	    wp_enqueue_script( 'keydesign_wp_admin_js', LEADENGINE_THEME_URI . 'core/assets/js/admin-scripts.js', '', '1.0.0' );
		}
	}
...
// ------------------------------------------------------------------------
	...
	if( ! function_exists( 'leadengine_dynamic_style' ) ) {
		function leadengine_dynamic_style() {
			ob_start();
			include_once( LEADENGINE_THEME_DIR . 'core/dynamic-styles.css.php' );
...
// ------------------------------------------------------------------------
	if( class_exists( 'KEYDESIGN_ADDON_CLASS' ) ) {
		function leadengine_embed_oembed_html($html, $url, $args) {
			global $post;
		...
		add_filter('embed_oembed_html','leadengine_embed_oembed_html', 10, 3);
	}
